...
def validate_inspect(it):
    ...
    labels = torch.randint(low=0, high=args.num_classes, size=(args.num_samples,)).to(args.device)
    x = model.sample(z, labels, args.sample_num_points, flexibility=args.flexibility) #, truncate_std=args.truncate_std)
...
def test(it):
    ...
    for i in tqdm(range(0, math.ceil(args.test_size / args.val_batch_size)), 'Generate'):
        with torch.no_grad():
            ...
            labels = torch.randint(low=0, high=args.num_classes, size=(args.val_batch_size,)).to(args.device)
            x = model.sample(z, labels, args.sample_num_points, flexibility=args.flexibility)
